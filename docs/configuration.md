...
这几个变量只影响 **后台推流页面上展示给主播的 RTMP / SRT 地址**，
不会改变 SRS 路由行为。**WHIP 推流 URL 始终使用 `PUBLIC_BASE_URL`**，
因为 WebRTC 必须直连源站，不能经过 CDN（详见 [`cdn.md`](./cdn.md)）。
...
| `OFFLINE_PLACEHOLDER_URL` | (空)                                                 | 全局离线占位内容 URL（可在管理后台覆盖） |

> 这些配置在 **管理后台 → 设置 → 品牌** 里也可以改，且会持久化到
...
当直播间未开播时，可以配置播放占位内容（图片、视频、音频或另一个直播流）。

**配置层级：**

1. **全局默认**：在 **管理后台 → 设置 → 品牌** 中配置 `全局离线占位内容 URL`，作为所有直播间的默认值
2. **单独配置**：在 **管理后台 → 直播间管理 → 直播间详情** 中为每个直播间单独配置 `离线占位内容 URL`，会覆盖全局默认值

**支持的媒体类型：**

- **视频**：`.mp4`, `.webm`, `.ogg`, `.mov` — 自动循环播放，静音
- **音频**：`.mp3`, `.wav`, `.aac`, `.m4a` — 自动循环播放
- **图片**：`.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`, `.svg` — 静态显示
- **直播流**：其他 URL（如 HLS、FLV 流地址）— 作为直播流播放

**切换逻辑：**

- **下播 → 离线占位**：直播间下播后，等待 **5 分钟**，如果仍未上线则自动切换到离线占位内容
- **上播 → 正常直播**：直播间上播后，**立即**切换回正常直播流
- **初次加载**：如果直播间已经离线，则直接显示离线占位内容（无需等待 5 分钟）

> 5 分钟的延迟设计是为了避免短暂的推流中断（如主播切换 OBS 场景、网络抖动）导致频繁切换。
